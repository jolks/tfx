...
class RuntimeParameter(json_utils.Jsonable):
  ...
  def __new__(
    ...
    if ptype and ptype != Text:
      raise TypeError(
          'Currenlty only support string-typed placeholder parameters')
    ...
    return '{{<RuntimeParameter>: %s</RuntimeParameter>}}' % json_utils.dumps(
        instance)
  ...
  # object.
  @classmethod
  def from_json_dict(cls, dict_data: Dict[Text, Any]) -> 'RuntimeParameter':
    """Convert from dictionary data to a RuntimeParameter."""
    instance = object.__new__(cls)
    instance.__dict__ = dict_data
    return instance

  @classmethod
  def parse(cls, placeholder: Text) -> 'RuntimeParameter':
    """Converts a placeholder generated by __new__ to a RuntimeParameter obj.
    ...
    return json_utils.loads(placeholder)
